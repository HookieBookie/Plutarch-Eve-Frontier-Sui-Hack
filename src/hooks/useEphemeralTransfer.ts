...
// ─────────────────────────────────────────────────────────────────────────────
// usePickupBatch — combined release_to_main + claim_supply in ONE PTB
// Avoids the OwnerCap version mismatch that occurs with two separate txns.
// ─────────────────────────────────────────────────────────────────────────────

export function usePickupBatch(ssuId: string | undefined) {
  const { signAndExecuteTransaction } = useDAppKit();
  const account = useCurrentAccount();
  const [state, setState] = useState<TxResult>({ pending: false, error: null, digest: null });

  /**
   * @param toRelease — items to move from open storage → main (corporate release)
   * @param toClaim   — items to move from main → ephemeral (pickup)
   *                     If empty, only releases are performed.
   */
  async function pickupBatch(
    characterId: string,
    ownerCapId: string,
    toRelease: { typeId: number; quantity: number }[],
    toClaim: { typeId: number; quantity: number }[],
  ): Promise<string | null> {
    if (!account || !ssuId) {
      setState({ pending: false, error: "Wallet or SSU not available", digest: null });
      return null;
    }
    if (toRelease.length === 0 && toClaim.length === 0) return "noop";
    setState({ pending: true, error: null, digest: null });
    try {
      const W = worldPkg();
      const ssuObjectId = await resolveSsuObjectId(ssuId);
      ...
      // First: release items from open storage → main
      ...
      // Then: claim items from main → ephemeral
